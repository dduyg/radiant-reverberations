...
let upperHemisphereLightSourceX = width * 0.1;
let upperHemisphereLightSourceY = height * 0.1;
let lowerHemisphereLightSourceX = width * 0.9;
let lowerHemisphereLightSourceY = height * 0.9;
...
function draw() {
  ...
  for (let phi = 0.0; phi < 2.0 * PI; phi += sampleAngularDelta) {
    for (let theta = 0.0; theta < 0.5 * PI; theta += sampleAngularDelta) {
      ...
      const sampleX = x * sphereRadius + sphereCenterX + upperHemisphereLightSourceX;
      const sampleY = upperHemisphereCenterY + (z - y * 0.25) * sphereRadius + upperHemisphereLightSourceY;
  ...
  for (let phi = 0.0; phi < 2.0 * PI; phi += sampleAngularDelta) {
    for (let theta = 0.0; theta < 0.5 * PI; theta += sampleAngularDelta) {
      ...
      const sampleX = x * sphereRadius + sphereCenterX + lowerHemisphereLightSourceX;
      const sampleY = lowerHemisphereCenterY - (z + y * 0.25) * sphereRadius + lowerHemisphereLightSourceY;
